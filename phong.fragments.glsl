...
void main( void )
{
    vec3 L; // vecteur vers la source lumineuse
    ...
    vec4 texel = texture( laTextureCoul, AttribsIn.texCoord );

    if (length(texel.rgb) < 0.5 && iTexCoul > 0) {
		discard;
	} else if (iTexCoul != 0) {
        coul *= texel;
    }
    ...
    int j = 0;
    for (j; j<3; j++) {
         L = normalize(AttribsIn.lumiDir[j]);
